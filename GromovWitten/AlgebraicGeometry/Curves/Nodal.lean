...
/-! ## The relative differential Fitting locus on node charts

`GromovWitten/AlgebraicGeometry/FittingIdealsSheaf.lean` attaches to every relative scheme over an
affine base a genuine quasi-coherent ideal sheaf whose value on an affine open `V` of the source
is the differential Fitting ideal of `Ω[Γ(X, V)⁄Γ(Y, ⊤)]`.  The results below compute this ideal
in degree one on the étale node charts of a nodal curve over a field: it is generated by the two
pulled-back node coordinates `x` and `y`, so that the first Fitting locus is exactly the locus of
nodes on such a chart. -/

section FittingLocus

/-- The standard local-node algebra is of finite presentation over its coefficient ring. -/
instance localNodeRing_finitePresentation (R : Type u) [CommRing R] (π : R) (n : ℕ) :
    _root_.Algebra.FinitePresentation R (StableReduction.LocalNode.Ring R π n) :=
  _root_.Algebra.Presentation.finitePresentation_of_isFinite
    (StableReduction.LocalNode.completeIntersectionPresentation R π n)

/-- The affine local node is locally of finite presentation over its base. -/
instance localNode_locallyOfFinitePresentation (R : Type u) [CommRing R] (π : R) (n : ℕ) :
    LocallyOfFinitePresentation (StableReduction.LocalNode.toBaseSpec R π n) := by
  rw [StableReduction.LocalNode.toBaseSpec,
    HasRingHomProperty.Spec_iff (P := @LocallyOfFinitePresentation)]
  exact RingHom.finitePresentation_algebraMap.mpr inferInstance

/-- Presentation-free form of the local node computation: the first differential Fitting ideal of
`R[x, y]/(xy - πⁿ)` is the ideal generated by `x` and `y`. -/
theorem differentialFittingIdeal_one_localNode (R : Type u) [CommRing R] (π : R) (n : ℕ) :
    Algebra.differentialFittingIdeal R (StableReduction.LocalNode.Ring R π n) 1 =
      Ideal.span {StableReduction.LocalNode.x R π n, StableReduction.LocalNode.y R π n} := by
  rw [Algebra.differentialFittingIdeal_eq_of_presentation
      (StableReduction.LocalNode.completeIntersectionPresentation R π n) 1,
    StableReduction.LocalNode.differentialFittingIdeal_one_eq_relativeJacobianIdeal,
    StableReduction.LocalNode.relativeJacobianIdeal_eq]

/-- The relative differential Fitting ideal of the affine local node on its global sections,
transported along the canonical isomorphism `Γ(Spec B, ⊤) ≅ B`. -/
theorem map_idealOn_top_localNode (R : Type u) [CommRing R] (π : R) (n i : ℕ) :
    Ideal.map (Scheme.ΓSpecIso (.of (StableReduction.LocalNode.Ring R π n))).hom.hom
        (RelativeFittingLocus.idealOn (StableReduction.LocalNode.toBaseSpec R π n) i
          ⟨⊤, isAffineOpen_top _⟩) =
      Algebra.differentialFittingIdeal R (StableReduction.LocalNode.Ring R π n) i := by
  have hlfp : LocallyOfFinitePresentation
      (Spec.map (CommRingCat.ofHom (algebraMap R (StableReduction.LocalNode.Ring R π n)))) :=
    localNode_locallyOfFinitePresentation R π n
  rw [← Algebra.differentialFittingIdeal_toAlgebra, StableReduction.LocalNode.toBaseSpec]
  exact RelativeFittingLocus.map_idealOn_top_specMap
    (CommRingCat.ofHom (algebraMap R (StableReduction.LocalNode.Ring R π n)))
    (RingHom.finitePresentation_algebraMap.mpr inferInstance) i

/-- The relative differential Fitting ideal of the affine local node, written as the image of the
explicit ideal `(x, y)` under `B ≅ Γ(Spec B, ⊤)`. -/
theorem idealOn_top_localNode (R : Type u) [CommRing R] (π : R) (n : ℕ) :
    RelativeFittingLocus.idealOn (StableReduction.LocalNode.toBaseSpec R π n) 1
        ⟨⊤, isAffineOpen_top _⟩ =
      Ideal.map (Scheme.ΓSpecIso (.of (StableReduction.LocalNode.Ring R π n))).inv.hom
        (Ideal.span {StableReduction.LocalNode.x R π n, StableReduction.LocalNode.y R π n}) := by
  rw [← differentialFittingIdeal_one_localNode, ← map_idealOn_top_localNode R π n 1,
    Ideal.map_map]
  have hcomp : (Scheme.ΓSpecIso (.of (StableReduction.LocalNode.Ring R π n))).inv.hom.comp
      (Scheme.ΓSpecIso (.of (StableReduction.LocalNode.Ring R π n))).hom.hom = RingHom.id _ :=
    congrArg CommRingCat.Hom.hom
      (Scheme.ΓSpecIso (.of (StableReduction.LocalNode.Ring R π n))).hom_inv_id
  rw [hcomp, Ideal.map_id]

variable {K : Type u} [Field K] {g : X ⟶ Spec (.of K)} {p : X}

/-- The comparison ring map from the standard node algebra to the sections over an affine open of
an étale node chart. -/
def NodeChartAt.chartMap (c : NodeChartAt g p) (W : c.source.affineOpens) :
    StableReduction.LocalNode.Ring K 0 1 →+* Γ(c.source, W.1) :=
  ((Scheme.ΓSpecIso (.of (StableReduction.LocalNode.Ring K 0 1))).inv ≫
    c.toNode.appLE ⊤ W.1 (RelativeFittingLocus.le_preimage_top c.toNode W.1)).hom

/-- **Nodal criterion on a node chart.**  On every affine open of an étale node chart, the first
relative differential Fitting ideal of the family is generated by the two pulled-back node
coordinates.  In particular the first Fitting locus of the family restricts, on such a chart, to
the pullback of the singular locus `(x, y)` of the standard node. -/
theorem NodeChartAt.idealOn_one (c : NodeChartAt g p) (W : c.source.affineOpens) :
    RelativeFittingLocus.idealOn (c.toCurve ≫ g) 1 W =
      Ideal.span {c.chartMap W (StableReduction.LocalNode.x K 0 1),
        c.chartMap W (StableReduction.LocalNode.y K 0 1)} := by
  have hetale : Etale c.toNode := c.etale_toNode
  rw [c.overBase, standardNodeToSpec,
    RelativeFittingLocus.idealOn_comp_etale (StableReduction.LocalNode.toBaseSpec K 0 1)
      c.toNode 1 ⟨⊤, isAffineOpen_top _⟩ W
      (RelativeFittingLocus.le_preimage_top c.toNode W.1),
    idealOn_top_localNode, Ideal.map_map, Ideal.map_span]
  rw [Set.image_pair]
  rfl
